...
func settings(w http.ResponseWriter, r *http.Request) {
	...
	if ip == remoteControlIp {
		...
		//swag := r.FormValue("title")
		//fmt.Println(swag)
		fmt.Println(r.Method)
		fmt.Println(r.FormValue("title"))

		http.ServeFile(w, r, "roryflixWebPages/settings.html")
	} else {
		http.ServeFile(w, r, "roryflixWebPages/settings.html")
	}
...
func remote(w http.ResponseWriter, r *http.Request) {
	...
	if r.Method == http.MethodPost && ip == remoteControlIp {
		...
		case "setReplayButton":
			os.Remove("resetMovie.sh")
			s := fmt.Sprintf("xdotool mousemove %d %d\nsleep 0.5\nxdotool click 1", x, y)
			CreateFile("resetMovie.sh", s)
			fmt.Println("works")
			return
		case "setPauseButton":
			os.Remove("hitPlay.sh")
                        s := fmt.Sprintf("xdotool mousemove %d %d\nsleep 0.5\nxdotool click 1", x, y)
                        CreateFile("hitPlay.sh", s)
			fmt.Println("works")
			return
		case "setRefreshButton":
			os.Remove("refresh.sh")
                        s := fmt.Sprintf("xdotool mousemove %d %d\nsleep 0.5\nxdotool click 1", x, y)
                        CreateFile("refresh.sh", s)
...
func main() {
	...
	dir := fmt.Sprintf(".%s", videosFolder)
	//folder := fmt.Sprintf("%s/", videosFolder)
...
func serveVideos(w http.ResponseWriter, r *http.Request) {
	//dir := "/home/zerocool/roryflixJanBuild(stable)/videos/"//fmt.Sprintf("./%s", videosFolder)
